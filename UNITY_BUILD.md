...
**6 of 8 R library packages now use unity builds** (methods, parallel, tcltk, grDevices, graphics, utils).

All conflicts resolved via preprocessor techniques (`#define`/`#undef`) in the unity files - **no original source files modified**. Only stats and tools remain non-unity due to complex conflicts (`.c` file includes, yacc parser conflicts).
